...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Login failed. Please verify your email or sign up.');
    } finally {
        ...
        {authMode === 'login' ? (
          <div>
            <p className="text-xs text-slate-600 mb-5 leading-relaxed">
              Sign in with your registered email address to access your subscriber dashboard or administrator portal.
            </p>
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 text-[11px] text-slate-600">
                <span className="font-semibold text-slate-800">Administrator Access:</span> Sign in with{' '}
                <button
                  type="button"
                  onClick={() => setLoginEmail('admin@digitalheroes.co.in')}
                  className="text-emerald-700 font-mono font-bold hover:underline"
                >
                  admin@digitalheroes.co.in
                </button>{' '}
                to access operations, draw simulations, and verification payouts.
              </div>
              ...
              <form onSubmit={handleSignupSubmit} className="space-y-3.5">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-slate-900 text-xs focus:outline-hidden focus:border-emerald-600 focus:bg-white"
                    placeholder="Leave blank for standard default security"
                  />
                </div>
